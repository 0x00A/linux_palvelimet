...
Tehtävässä pyydetään tekemään vanha arvioitava laboratoriaharjoitus uudelleen soveltuvin osin. Valitsin harjoituksen https://terokarvinen.com/2021/final-lab-for-linux-server-course/
...
Avataan SSH portti 22 ja otetaan palomuuri käyttöön:
```bash
...
Valitsemassani tehtävässä seuraava osio on luoda 5 eri käyttäjää, mielestäni tämä on jo tehty mm. edellisessä osiossa, jossa luotiin käyttäjä `testuser`. Tämä vaihe tehtävässä olisi vain toistaa sama asia uudelleen 4 kertaa, joten lasken tämän osan tehdyksi.
...
Tätä ei löydy tällä nimellä, löysin verkosta https://manpages.debian.org/testing/apache2-utils/ab.1.en.html joka viittaisi siihen että työkalu on osa `apache2-utils` pakettia.
...
Man sivuilta selviää, että `-n` määrittää tehtävien kokonaismäärän ja `-c` samanaikaisten yhteyksien määrän.
